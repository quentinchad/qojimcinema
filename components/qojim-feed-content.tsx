...
interface QojimFeedContentProps {
  initialReviews: any[]
  currentUserId?: string
}

export function QojimFeedContent({ initialReviews, currentUserId }: QojimFeedContentProps) {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (review) => review.movie_title.toLowerCase().includes(query) || review.content.toLowerCase().includes(query),
      )
    }
  ...
  // Modifier la fonction handlePageChange pour ajouter le défilement instantané en haut de la page
