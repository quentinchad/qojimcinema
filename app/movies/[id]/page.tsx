...
export default function MoviePage({ params, searchParams }: MoviePageProps) {
  ...
  const hasUserReview = useMemo(() => {
    return user && reviews.some((review) => review.user_id === user.id)
  }, [user, reviews])

  useEffect(() => {
    // Si l'ID commence par "tv-", rediriger vers la page des séries TV
    if (movieId.startsWith("tv-")) {
      const tvId = movieId.replace("tv-", "")
      router.replace(`/tv/${tvId}`)
      return
    }
  }, [movieId, router])
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        ...
        // Précharger les films similaires pour une navigation plus rapide
        fetch(`/api/tmdb/related/${movieData.id}`).catch((err) => {
          console.error("Error prefetching related movies:", err)
        })

        // Précharger les recommandations basées sur les genres
        fetch(`/api/tmdb/genre-recommendations/${movieData.id}`).catch((err) => {
          console.error("Error prefetching genre recommendations:", err)
        })
  ...
  const handleAddReviewClick = useCallback(() => {
    if (!user) {
      router.push(`/auth/login?redirectTo=${encodeURIComponent(`/movies/${movieId}?type=${type}`)}`)
      return
    }
    setShowAddReviewForm(true)
  }, [user, router, movieId, type])

  const handleReviewAdded = useCallback((newReview: any) => {
    setReviews((prevReviews) => [newReview, ...prevReviews])

    // Mettre à jour les listes de critiques
    if (newReview.users?.is_qojim) {
      setExpertReviews((prevReviews) => [newReview, ...prevReviews])
    } else {
  ...
  const handleReviewUpdated = useCallback((updatedReview: any) => {
    ...
    setReviews((prevReviews) => updateReviewInList(prevReviews))
    setExpertReviews((prevReviews) => updateReviewInList(prevReviews))
    setUserReviews((prevReviews) => updateReviewInList(prevReviews))
  }, [])
  ...
  const handleReviewDeleted = useCallback((reviewId: string) => {
    ...
    setReviews((prevReviews) => filterReviewFromList(prevReviews))
    setExpertReviews((prevReviews) => filterReviewFromList(prevReviews))
    setUserReviews((prevReviews) => filterReviewFromList(prevReviews))
  }, [])
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative w-full h-[60vh] rounded-b-lg overflow-hidden">
        <Image
          ...
          onError={(e) => {
            ...
            target.src = "/placeholder.svg?height=600&width=1200&text=No+Backdrop"
          }}
        />
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="relative aspect-[2/3] rounded-lg overflow-hidden mb-6">
              <Image
                ...
                onError={(e) => {
                  // Fallback en cas d'erreur de chargement
                  const target = e.target as HTMLImageElement
                  target.src = "/placeholder.svg?height=450&width=300&text=No+Poster"
                }}
              />
            </div>
        ...
        {/* Section des revues recommandées - chargée progressivement */}
        <ProgressiveHydration delay={100}>
          <Suspense fallback={<div className="py-8 animate-pulse h-64 bg-gray-800 rounded-lg"></div>}>
            <RelatedReviews
