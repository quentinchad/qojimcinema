...
export default function TVShowPage({ params }: TVShowPageProps) {
  ...
  const formattedTvId = `tv-${tvId}` // Ajouter le préfixe tv- pour les requêtes à notre API
  ...
  useEffect(() => {
    const fetchTVShowDetails = async () => {
      try {
        // Récupérer les détails de la série via notre API
        const response = await fetch(`/api/tmdb/movie-details/${formattedTvId}`)
        ...
        if (session?.user) {
          ...
          // Vérifier si la série est dans la liste d'attente
          const { data: watchlistData, error: watchlistError } = await supabase
            .from("watchlist")
            .select("id")
            .eq("movie_tmdb_id", formattedTvId)
            .eq("user_id", session.user.id)

          console.log("Watchlist check for TV show:", {
            tvId: formattedTvId,
    ...
    fetchTVShowDetails()
  }, [formattedTvId, supabase])

  const fetchReviews = async () => {
    try {
      // 1. Récupérer les critiques pour cette série
  ...
  const handleAddReviewClick = () => {
    if (!user) {
      router.push(`/auth/login?redirectTo=${encodeURIComponent(`/tv/${tvId}`)}`)
      return
    }
    ...
    return (
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="container mx-auto px-4 py-8 text-center">
          ...
          <p>{error || "Impossible de charger les détails de la série."}</p>
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative w-full h-[60vh] rounded-b-lg overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent">
          <div className="absolute bottom-0 left-0 p-8">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-2">{tvShow.title}</h1>
            <p className="text-gray-300">{tvShow.release_date?.substring(0, 4)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="space-y-4 bg-gray-900 p-4 rounded-lg mt-4">
              <div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-gray-400 text-sm ml-1">({tvShow.vote_count} votes)</span>
                </div>
              </div>

              {tvShow.episode_run_time && tvShow.episode_run_time.length > 0 && (
                <div>
                  <h3 className="font-semibold text-lg mb-1">Durée par épisode</h3>
                  <div className="flex items-center gap-1">
          ...
          <div className="md:col-span-2">
            <section className="mb-8">
              ...
              <p className="text-gray-300 leading-relaxed">
                {tvShow.overview || "Aucun synopsis disponible pour cette série."}
              </p>
            </section>
            ...
            {showAddReviewForm && (
              <section className="mb-8">
                <AddMovieReviewForm
                  ...
                  movieTitle={tvShow.title}
                  moviePosterPath={tvShow.poster_path}
                  movieBackdropPath={tvShow.backdrop_path}
                  userId={user.id}
        ...
        <RelatedReviews
          currentMovieId={formattedTvId}
          currentMovieTitle={tvShow.title}
          currentMovieGenres={tvShow.genres?.map((g: any) => g.id)}
        />
      </div>
    </div>
  )
